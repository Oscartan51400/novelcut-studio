...
guard CommandLine.arguments.count == 5,
  ...
  fputs("usage: render-title-card.swift output.png width height text.txt\n", stderr)
  exit(2)
}
...
image.lockFocus()
NSColor(calibratedRed: 0.035, green: 0.043, blue: 0.055, alpha: 1).setFill()
...
let specs: [(Double, NSFont.Weight, NSColor)] = [
  (min(72, width * 0.095), .bold, NSColor(calibratedRed: 0.86, green: 0.19, blue: 0.16, alpha: 1)),
  (min(38, width * 0.052), .medium, NSColor(calibratedWhite: 0.96, alpha: 1)),
  (min(28, width * 0.036), .regular, NSColor(calibratedWhite: 0.65, alpha: 1))
]
